...
func run() error {
	...
	handler := session.Handler(session.Config{
		...
		// web-client P2: BFS step bound for the Room.Map GMCP neighbourhood. 0
		// (default) = the whole area, matching the `map` verb; a positive value
		// bounds it for very large areas.
		RoomMapRadius: envIntOr("ANOTHERMUD_ROOM_MAP_RADIUS", 0),
		...
		// reputation.md §6: the look-at-player recognition check difficulty (flat
		// v1; per-location difficulties deferred). Renown ≥ this is reliably
		// recognized; below it needs a high die.
		RecognitionDifficulty: envIntOr("ANOTHERMUD_RECOGNITION_DIFFICULTY", 100),
