...
func run(ctx context.Context, c conn.Connection, cfg Config) error {
	...
	spawnView := command.RenderRoom(start, cfg.Placement, cfg.Items, questMarkerFor(cfg.Quests, a.PlayerID()), cfg.Ambience, nil, startLvl, exitVisibleFor(a, cfg.AdminRole), command.QuestSpawnVisible(a, cfg.AdminRole), otherPlayerNames(cfg.Manager, start.ID, a.PlayerID())...)
