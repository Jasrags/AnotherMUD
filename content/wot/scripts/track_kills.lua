-- track_kills.lua — pack scripting demo (scripting-and-packs).
--
-- Subscribes to mob.killed and logs every kill through the engine logger.
-- Demonstrates the minimal scripting surface: subscribe to a bus event by
-- name, receive the payload as a Lua table with snake_case field names, and
-- emit a log line via engine.log.
--
...
engine.subscribe("mob.killed", function(name, p)
  ...
  -- Schedule a one-shot follow-up ~3s after the kill (30 ticks @ 100ms).
  -- Event-gated so it only fires after a real kill, never continuously.
  -- Captures mob_name in the closure.
