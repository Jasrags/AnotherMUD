...
- the tick loop + event bus
- rooms/areas/exits with **doors, locks, temporary portals, weather, an in-game clock, and per-viewer light/darkness**
- the entity store (items + mobs, double-buffered tag index)
- inventory/equipment/slots/stacking/keyword resolution
- progression (stats/races/classes/tracks/alignment/training/**proficiency with use-based gain**/**abilities**/**effects**)
- combat (engage/round/hit-miss-damage/flee/death/**corpses+looting+decay**)
- mobs + area spawning + AI + loot
- economy (currency/shops/**sustenance**/**rest**/**consumables**)
- **crafting & cooking + gathering (forage/harvest) over biome resource tables**
- **room coordinates + an ASCII `map`/minimap with persisted fog-of-war**
- quests
- the command registry with **typed arguments** (M17.2)
- a **sandboxed Lua scripting runtime** (gopher-lua) with hot reload
- social (notifications/tells/channels/emotes)
- accounts + versioned player saves + an interactive **character-creation wizard** + sessions (flood/idle/link-dead/takeover)
- **telnet with full IAC negotiation + WebSocket + GMCP packages + tiered ANSI color**
...
- **Saves on disk:** `accounts/index.yaml` (email→id + username→id maps; login authenticates by username, email kept for back-compat), `accounts/<id>/account.yaml`, `players/<lowercased-name>/player.yaml` (+ sibling `quest.yaml`, `notifications.yaml`, chat-subscriptions), and `channels/<id>.yaml` (channel scrollback), all under `ANOTHERMUD_SAVE_DIR`. Writes use the tmp→bak→rename rotation in `internal/persistence`. Player saves carry a `version`; `player.CurrentVersion` is **39**, with an **append-only** migration chain — never edit an existing migration, add a new one and bump. The save holds tags/roles, stats (incl. persisted base), properties, equipment/inventory, abilities + proficiencies, recall address, prompt template, fog-of-war set, known recipes, feats + credits, resource-pool currents (mana/movement/One Power), gender, `WorldID` (world-locking), `ChannelingGift`, background choices, known languages, faction standing, reputation, owned mounts + hirelings, the `AdminTags` bag, the contextual-tips opt-out + shown-once set, per-item burned-SIN/credential flags (v37), security heat + wanted-level state (v38), and the karma ledger (v39). **Per-version detail — what each vNN added and its migration — lives in the `internal/player` migration chain and `docs/ROADMAP.md`; it is deliberately not duplicated here.**
...
actually demands deeper reasoning. Default to **`claude-sonnet-5`** and
