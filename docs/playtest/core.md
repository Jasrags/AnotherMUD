# AnotherMUD Playtest — Core / Starter-World

A manual QA checklist for the **engine-mechanics showcase** on the
core/starter-world demo (the fantasy fighter village). This is the default boot
(`make run`); the **Wheel of Time** (`wot.md`) and **Shadowrun** (`shadowrun.md`)
guides are siblings — see [README.md](README.md) for the split and the shared
conventions.

This guide covers **§0–§26, §28–§31, §36**. Section numbers are guide-wide
anchors (they don't renumber per file), so a few numbers here jump — the missing
ones (§27 channeling, §32 masterwork, §34 faction, §35 reputation) live in
`wot.md`. **§33 (ranged combat)** is written up in `wot.md`, but its *thrown*
half runs on **this** default boot.
