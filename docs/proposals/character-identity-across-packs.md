...
**Status:** **SHIPPED 2026-06-16** — spec `docs/specs/character-identity.md` +
code (manifest `kind: world|library`, save **v23** `WorldID` backfilled from the
`Location` namespace, creation stamp + login/roster gate, boot guard). §7 resolved
(all six recommended defaults adopted — see §7). This proposal remains the design
rationale (why world-locking, the portable/content-bound split); the spec is the
authority for behavior. Co-host + global-registry namespacing (§7-Q6) deferred.
