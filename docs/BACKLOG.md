...
| Item modification (capacity + mounts) | **item-modification §1–11** + **weapon-accessories §1–9** (new) | No capacity/mod concept anywhere (`internal/item`, `internal/entities`); item modification is an explicit non-goal of `inventory-equipment-items`. **Two slices on one substrate:** (A) armor **capacity** budget — host `Capacity` + mod `CapacityCost`, `modify` install/remove verb, equip-time aggregation that folds mods into the wearer's contribution incl. the **typed depth fields** (armor bonus / resistances, not only `[]Modifier` — the load-bearing gotcha), durable installed-mods on `ItemInstance` → **player save-version bump**; (B) weapon **mount-slot** admission (named barrel/under-barrel/side/top/stock/internal mounts, one accessory each) reusing A's substrate. Scope: **Core-source** armor+mods (A) / weapons+accessories (B) only; cyberware clusters are a later capacity host domain (same rule). v1 models each rating as a distinct template (flat cost, no runtime rating). Smartgun↔smartlink pairing deferred |
