...
| Property-registry save-pipeline integration | persistence §2 / §4.4 | **NOT actually ready — trigger unfired (verified 2026-07-17).** Registry substrate exists (M14.4) but `property.Wrap`/`Unwrap` have **zero production callers**; `InventoryEntry` uses bespoke fields (`Loaded`/`Grade`/`Mods`/`Burned`), not a general property bag. Building the generic pipeline now is speculative generality. Its own deferral trigger ([[m14-deferred-fixes]]): "the moment any existing save shape needs a **content-author-declared** property (vs. a hardcoded field)." Build it then, driven by that real first consumer — not before. |
...
- **Prompt template polish (from the adaptive-prompt work, 2026-07-17)** — the
  **default** prompt is pool-adaptive (`render.DefaultPromptTemplate(PromptVitals)`:
  HP/MV always, ST/MA gated on the stun/mana pool existing). **SHIPPED since:**
  conditional-segment syntax for player-set _custom_ templates — `{?pool}…{/pool}`
  wrappers (case-insensitive, gated on the pool's max > 0) so a hand-written template
  adapts like the default (`ui-rendering-help.md` §7.5; `render.renderPromptInto`).
  **Still deferred: a currency segment in the default** — skipped in favor of the
...
- **SINs / SINless + legality gating** — **✅ SLICES 1 + 2 + 3 SHIPPED (2026-07-16, `sin-and-legality.md`).**
  ...
  **credential** items (fake SINs) whose `permits` clear the gate — SR content skins it as
  SINs/licenses (`sin`/`licenses` verb), engine stays setting-agnostic. The pre-decision "SIN
  as attribute vs. item" resolved to **item**. **Slice 2 — verification / burn:** a
  `scanner_rating` storefront rolls `d20 + credential_rating` vs the scanner on a **restricted**
  purchase; a failure **burns** the fake (`InventoryEntry.Burned`, save **v37**, re-hydrated at
...
| **Web client (enriched gameplay)** — *stated direction 2026-07-08; Mudlet HUD paused in favor of this* | a browser front-end over the **existing** `internal/conn/ws` WebSocket + `{type,package,data}` GMCP JSON envelope: interactive coordinate map (reuse `Room.Info` x/y/z — validated), drag-drop inventory, crafting/trade forms, real-time HUD, portraits, responsive/mobile. Natural home for the web-admin console above. | **The transport + all 11 engine GMCP packages already exist and are reused wholesale** — only the small Mudlet-specific Lua is set aside (it did its job: proved the coord data is correct). **Decision-0 fork RESOLVED (2026-07-15) → `docs/themes/web-client-plan.md`:** chose **C — GMCP baseline + additive rich-client messages** (web client = superset, telnet never breaks, engine sole authority). Mechanism pinned: additive messages ride **new GMCP packages in existing namespaces** (not new envelope types), inbound intents **reduce to existing engine commands**, no handshake to start (ignore-unknown), ruleset-agnostic wire. First deliverable = **P1** browser shell consuming the existing 11 packages with **zero server change**. **Prereq DONE:** generalized pool currents on `Char.Vitals` (the `pools` map, `dc07fe0`). |
