...
|---|---|---|---|---|---|---|
| **S1** | **Weapon & Equipment Depth** | proficiency tiers, crit, damage types, ranged, armor, size-wield | `item`/`combat`/`slot` | **A+B+C (`M-Weapon-Identity`) SHIPPED 2026-06-10** (category/tier/damage-type metadata, class-granted proficiency + non-proficient penalty, per-weapon crit threat/multiplier, + a demo). **F (size-wield) slices 1+2 SHIPPED 2026-06-17** (`size-and-wielding.md`, `internal/size` leaf pkg) — slice 1: size substrate + size-relative wield mode (`Mode`→light/one-/two-handed/too-large by ordered distance) + the two-handed Strength bonus (1.5× Str term); slice 2: equip footprint DERIVED from wield mode (too-large refused, two-handed ties up the off hand, light/one-handed leaves it free; a weapon with no `size:` keeps its static companion slots); slice 3: MOBS get the same size-relative two-handed Strength in combat (`MobInstance.Stats`), so a large mob one-hands — no bonus — what a medium player two-hands (§5). Demo: large ashandarei (two-handed) + huge Trolloc maul (refused) in the Smithy. Remaining F: §4.3 light off-hand eligibility + two-weapon attacks (deferred to its own increment); unarmed-as-light is behaviorally satisfied (no weapon ⇒ no bonus). **E (armor) §3+§5+§6 SHIPPED 2026-06-16** (`armor-depth.md`) — AC composition (armor bonus + max-Dex cap via the WoT-only `defense: ac + dex_ac` channel-map override; baseline AC unchanged) + armor proficiency (class `armor_proficiency_tiers`, non-proficient check-penalty extended to attack rolls + clumsy cue) + the §6 attack-roll completion; §4 per-type mitigation (D, damage types) shipped earlier with masterwork; §7 don/doff timers deferred (instant default). AC-model fork RESOLVED: single AC on `defense` (decompose-and-cap, Dex term WoT-only) + per-type resistance on the `mitigation` channel (the cross-ruleset soak primitive — WoT leans `defense`, Shadowrun leans `mitigation`). **H (masterwork) SHIPPED 2026-06-16** (`masterwork.md`, `internal/grade`: graded quality bonuses via existing seams, grade kept independent of the rarity decoration). **G (ranged) SHIPPED 2026-06-16** (`ranged-combat.md`) — Slice A (thrown/projectile + ammo + Strength rules + masterwork ammo) + Slice B (far→near→melee range bands, auto-close + kiting, incl. mob ranged AI) + **Model C** (cross-room targeting as an opportunistic adjacent-room action: the `shoot` verb + a shot mob's retaliation pursuit; sustained cross-room combat + multi-room LoS/pursuit deferred per §10). **K (two-weapon fighting) COMPLETE — slices 1-4 SHIPPED 2026-06-17** (`two-weapon-fighting.md`): (1) off-hand attack substrate — a light off-hand weapon (consumes F §4.3) grants an extra off-hand swing (`combat.Stats.OffHand`, existing per-swing machinery) with its own dice/crit/type, a two-weapon to-hit penalty on both hands, ½× off-hand Strength; slot-aware pick (main=wield, off=offhand; spanning two-hander excluded). (2) the feats — Two-Weapon Fighting (`two_weapon_hit`) + Ambidexterity (`off_hand_hit`) subtractively reduce the penalties (-4/-8 → -2/-2) via the feat-bonus cache. (3) Improved Two-Weapon Fighting (`off_hand_attack`) — a SECOND off-hand strike at a cumulative secondary penalty (`SecondaryOffHandPenalty`, the source's -5; first iterative-attack penalty the engine models — main-hand extras stay flat). (4) mob dual-wield — content-driven (a 2nd light offhand-eligible spawn weapon), un-feated (full penalty, one strike), combat loop unchanged (`MobInstance.Stats`/`SetOffWeapon` + `EquipMobAtSpawn` off-hand detection). Demos: belt knife in the Smithy (player) + brigand-cutthroat on the Quarry Road (mob). NOTHING deferred. Remaining S1: armor §7 don/doff timers, encumbrance (I), special weapons (J) | M done; L remains | spec `weapon-identity.md` + `size-and-wielding.md` + `armor-depth.md` + `masterwork.md` + `ranged-combat.md` + `two-weapon-fighting.md` |
| **S2** | **The One Power (channeling)** | saidin/saidar, weaves, daily slots, affinities, talents, overchannel, linking, madness | `progression` abilities + `effect`; **resource-pool substrate SHIPPED + specced** | **Proposal drafted 2026-06-11** ([`proposals/wot-the-one-power.md`](../proposals/wot-the-one-power.md)) — D1 single Power pool (not slots), D2 weaves = `spell` abilities + use-based proficiency, D3 gendered Source / defer taint, D4 one channeler class (split-seam free), D6 overchannel = Fort save (S6) → cascade (S5). Phasing: P0 Power-pool substrate (closes BACKLOG "Mana pool") → P1 channeler class + starter weaves + `channel` verb → P2 overchannel → P3+ affinities/interrupt-game/taint/angreal/linking. Research: [`research/wotmud-channeling.md`](../research/wotmud-channeling.md). **Phases 0–3 SHIPPED 2026-06-14** (save v22): pool substrate + persistence + regen + spend knobs (P0); channeler class + One Power pool + classic-four weaves + `channel` verb (P1); `overchannel` → Fort save → fatigued/stunned/stilled cascade + channel-block (P2); **affinities & the Five Powers (P3)** — `elements` field on abilities + gender at creation (v22) + gender-derived two-tier affinity (men Earth/Fire/Spirit, women Air/Water/Spirit) → soft potency scaling of weave damage/heal, weakest element governing. **P4 partially SHIPPED 2026-06-14:** affinity now also scales the EFFECT path (save-gated entry DC + recurring-save DC + buff-modifier magnitudes, via an injected `PotencyFunc` on the resolver — affinity coverage COMPLETE); and the **interrupt game is DONE** — weaves take an interruptible `cast_time` warmup (in combat rounds; `CastTracker`/`CastNotifier` + the ability-phase begin/advance/resolve state machine), and a **hit, a room change, or an incapacitating condition (stun)** all abort an in-flight weave (free refund — under spend-on-success an unresolved cast never drew the Power). **Initiate/Wilder split also DONE 2026-06-14** — the single `channeler` class replaced by `initiate` (INT-keyed pool, weak Fort) + `wilder` (WIS-keyed pool, strong Fort + bigger HP die); content-only (the divergence rides existing growth-bonus + save-progression seams, zero engine code). **Angreal/sa'angreal SHIPPED 2026-06-17** — a same-gender held (equipped) device amplifies woven damage/heal (the engine analog of the d20 "extra effective casting level" → weave magnitude); typed `angreal_power`/`angreal_gender`, strongest-equipped-wins (no stacking), `ANOTHERMUD_ANGREAL_PER_POINT` (default 0.25), rides the affinity damage/heal scaler (effect-path save-DC/buff amplification deferred — the resolver's `PotencyFunc` stays weaken-only). P4+ remaining: Talents catalogs (not ported — d20 bookkeeping), Wilder emotional Block, taint/madness, linking, stilling restore path (+ optional interrupt polish: GMCP cast-bar, cast_time tuning). **Engine substrate now SPECCED (setting-agnostic, 2026-06-15):** generalized resource pools (`progression.md` §2.6 + the player-save surface in the specs README), the cast-time/interrupt game + the reserve-gate/overexertion pattern (`abilities-and-effects.md` §4.9–§4.10), and the channel-map derived-formula seam (`combat.md` §4.4); [`wot-the-one-power.md`](../proposals/wot-the-one-power.md) is now scoped to the WoT-specific instantiation (saidin/saidar, Five Powers, taint, Initiate/Wilder). | **XL** | **P0–P3 + P4 (affinity-complete + interrupt game + Initiate/Wilder split) + angreal shipped + substrate specced; P4+ depth open** |
