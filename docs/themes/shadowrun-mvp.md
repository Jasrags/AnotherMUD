...
> **STATUS: SHIPPED 2026-07-08.** As scoped, this was finish-and-prove, not greenfield: the engine's ranged/ammo system already shipped and was fully wired (the `AmmoFor` round-loop hook `main.go:2064` → `session.AmmoConsumer` spends one ammo unit per projectile swing and skips the swing dry via `OnRangedDry`; range bands + melee-band penalty; `throw`/`shoot`/`load` verbs), and the SR firearms were already authored (`heavy-pistol`/`smg` `ranged_class: projectile` + `ammo_kind: bullet`). `TestLive_ShadowrunFirearm` proves it live — a clipless heavy pistol clicks empty in melee ("isn't loaded — reload first"; a holder-fed/magazine firearm reads as *unloaded*, distinct from the loose-ammo *dry* "no {ammo} left to shoot" line a bow gives), then fed with rounds it fires point-blank and hits the ganger on the Physical monitor (soak applied, no target_pool). The clip-vs-round call resolved **Option A** (per-shot round): `caseless-round` renamed to "a caseless round" (each stackable unit = one shot, the arrow model; id kept so shop/room refs hold). The **magazine model (Option B)** shipped as its own slice — **SR-M3e** below; SMG burst-fire and SR cross-room `shoot` remain deferred.
...
- [x] With no clip / an empty magazine, the shot clicks empty (`RangedDry`, `Unloaded`) and is skipped, narrated to the player ("isn't loaded — reload first"). (The loose-ammo *dry* line, "no {ammo} left to shoot", is a bow drawing an empty quiver — a firearm always reads *unloaded*, fix = reload.)
