...
- [personal-aliases](personal-aliases.md) — per-character, player-defined
  command aliases with `$`-parameter substitution, expanded server-side in a
  pre-parse stage before the §4 parser (loop-safe, chain-cap bounded); the
  `alias`/`unalias` verbs, per-character persistence + append-only migration,
  and a `Char.Aliases` GMCP package that roams the table to Mudlet + the web
  client (client edits reduce to the verbs — authority invariant). *(Spec ahead
  of code — build pending. Resolves the server-side + GMCP parity fork in
  `../clients/parity-matrix.md`. Sibling of tab-completion.)*
