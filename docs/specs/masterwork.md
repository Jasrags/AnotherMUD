...
<!-- Spec style: narrative + acceptance criteria · Detail level: behavior only · Status: PARTIAL — EPIC S1 increment H. SHIPPED slices 1+2 (2026-06-16): the grade vocabulary substrate (internal/grade, pack-loaded `grades:`), the grade on item template + instance, the WEAPON to-hit bonus (§3), the POWER-WROUGHT flat damage bonus composed into the `damage_bonus` channel via a `damage_mod` stat post-crit (§3), and the unbreakable flag carried inert on the power-wrought grade (§4), and the ARMOR/SHIELD check-penalty improvement (§3) — a graded armor reduces its check penalty (floored at zero) via the armor_check stat the skill check subtracts (this also built armor-depth §6's skill-check consumer). All as grade-scoped equip modifiers (§5 independence, §6 persistence). PLUS crafting→grade (§7/§9): a high-quality craft stamps a quality grade on the result via the output template's `quality_grades` tier→grade map (mirrors `quality_effects`), so a fine craft is mechanically graded and confers the equip bonuses (the wot iron-dagger grades from a fine forge). PLUS the tool skill-check kind (§3): a carried tool declaring `skill_tool: <skill>` aids that skill check by its base `skill_tool_bonus` + the grade's ToolSkill, best-applies (non-stacking) — this also built the skills.md tool seam (the `pick`/Open-Lock consumer; a starter-world lockpick demos it). **COMPLETE — all five increment-H kinds shipped (weapon to-hit, power-wrought damage + unbreakable, armor check-penalty, crafting→grade, tool skill-check).** Boot-validated: both the template `grade` field and `quality_grades` values resolve against the registry. grade (mechanical) kept independent of decoration (cosmetic) per item-decorations §1.1 -->
