...
Follow listens to that signal: when a **leader** arrives in a new room, each of
their **followers** attempts to make the same trip.
...
- **Death** of the leader: death relocates the leader (respawn / corpse, a
  non-adjacent move), which breaks the follow by the §3 rule.
...
- **Future consumers:** **grouping** (party reward-sharing over this primitive),
  **hireable mobs** (a mob follower), and the **onboarding guide** (a guide that
  follows a newbie) — each its own spec; this defines the shared move-with-leader
  mechanic they all need.
